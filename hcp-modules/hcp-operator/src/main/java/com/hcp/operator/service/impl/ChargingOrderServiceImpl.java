...
//        String orderId = pileId.concat(port).concat(LocalDateTimeUtil.format(LocalDateTime.now(),"yyyyMMddHHmmss")).concat(Seq.getId(new AtomicInteger(1),2));
     ...
     * 3. 费率明细写入
     */
    private void processV2RateSettlement(ChargingOrder order, String startTime,
                                         String endTime, Double totalPower) {
        // 获取该桩的费率模板
    ...
    private void calcAndSaveHourlyEnergy(String orderId, DateTime chargeStart,
        ...
        while (cursor.isBefore(chargeEnd)) {
            ...
            // 计算该slot在充电时段内的有效分钟数
            DateTime effectiveStart = cursor.isAfter(chargeStart) ? cursor : chargeStart;
            DateTime effectiveEnd = slotEnd.isBefore(chargeEnd) ? slotEnd : chargeEnd;
            long effectiveMinutes = Math.min(
                    DateUtil.between(effectiveStart, effectiveEnd, DateUnit.MINUTE), 30
            );
    ...
    private void calcAndSaveOrderRateDetails(String orderId, DateTime chargeStart,
        ...
        for (int i = 0; i < feeRangeList.size(); i++) {
            ...
            detail.setRateIndex(i + 1);
            detail.setRatePrice(ratePrice);
            detail.setEnergy(BigDecimal.valueOf(segmentEnergy).setScale(4, RoundingMode.HALF_UP));
            detail.setLossEnergy(BigDecimal.ZERO); // 计损电量，按实际策略调整
            detail.setAmount(amount.setScale(4, RoundingMode.HALF_UP));
    ...
    private long getOverlapMinutes(DateTime chargeStart, DateTime chargeEnd,
        ...
        DateTime overlapStart = chargeStart.isAfter(rangeStart) ? chargeStart : rangeStart;
        DateTime overlapEnd = chargeEnd.isBefore(rangeEnd) ? chargeEnd : rangeEnd;
